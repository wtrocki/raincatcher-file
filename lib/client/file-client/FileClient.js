...
FileClient.prototype.init = function() {
  var self = this;
  this.fileQueueManager = new FileQueueManager(this.$fh, this);
...
FileClient.prototype.createFile = function(file) {
  if (file.fileURI && file.options) {
    ...
    return this.uploadDataUrl(file.userId, file.dataUrl, file.options.fileName);
  } else {
 ...
 * @returns {*}
 */
FileClient.prototype.sheduleFileToBeUploaded = function(file) {
  var self = this;
 ...
 * @param dataUrl
 */
FileClient.prototype.uploadDataUrl = function(userId, dataUrl, fileName) {
  var deferred = q.defer();
  if (arguments.length < 2) {
    deferred.reject('Both userId and a dataUrl parameters are required.');
  } else {
    this.$fh.cloud({
      path: config.apiPath + '/owner/'+userId+'/upload/base64/' + fileName,
      method: 'post',
      data: dataUrl
    },
    function(res) {
