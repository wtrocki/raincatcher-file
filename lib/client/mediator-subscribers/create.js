...
module.exports = function createFileSubscriber(fileEntityTopics, fileClient) {
  ...
  return function handleCreateFileTopic(parameters) {
    ...
    fileClient.sheduleFileToBeUploaded(fileToCreate).then(function(result) {
      self.mediator.publish(fileEntityTopics.getTopic(CONSTANTS.TOPICS.CREATE, CONSTANTS.DONE_PREFIX, parameters.topicUid), result);
