...
function initRouter(mediator) {
  ...
  router.route('/all').get(function(req, res) {
    mediator.publish('wfm:cloud:file:list');
    mediator.once('done:wfm:cloud:file:list', function(files) {
      res.json(files);
    });
  });
  ...
  router.route('/owner/:owner/upload/base64/:filename').post(function(req, res, next) {
    ...
    writeStreamToFile(fileMeta, stream).then(function(fileMeta) {
      return mediator.request('wfm:cloud:file:create', fileMeta);
    })
  ...
  function(req, res, next) {
    ...
    mediator.request('wfm:cloud:file:create', req.fileMeta)
    .then(function(fileMeta) {
      res.json(fileMeta);
    }, function(err) {
      console.error(err);
